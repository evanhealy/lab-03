---
title: "Lab 03 - Take a sad plot and make it better"

author: "Ross, Evan, Ashton, Addison, User5 and User6 - Wee Gits"
date: "`r Sys.Date()`"
output: html_document
---

### Load packages

```{r load-packages, message = FALSE, warning=FALSE}
library(tidyverse)
library(ggplot2)
```


# Questioning data visualisations

## Image 1

```{r echo=FALSE, out.width="100%"}
knitr::include_graphics("img/Speaking-the-world.jpg")
```

_Write your comments about image 1 below_

Image 1 data is not correctly scaled. 
It says 1,026 billion instead of 1.026 billion


## Image 2

```{r echo=FALSE, out.width="100%"}
knitr::include_graphics("img/rose-pop-density-total-area-bw.jpg")
```

_Write your comments about image 2 below_

Image 2 data is unlabeled and each country have different sizes.

## Image 3

```{r echo=FALSE, out.width="100%"}
knitr::include_graphics("img/life-expectancy-rule-of-law.png")
```

_Write your comments about image 3 below_
The image is unclear, and unreadable - in the overlap of the colours and the use of unusual acronyms.
There are no clear measurements on the axis - what does "Rule of Law" mean



## Image 4

```{r echo=FALSE, out.width="100%"}
knitr::include_graphics("img/global-income-dist.PNG")
```

the image is difficult to read, some countries are grouped together. X-axis does not follow a scale 





# Exercises

```{r load-data-staff, message=FALSE, eval=TRUE}
staff <- read_csv("data/instructional-staff.csv")
```


## Exercise 1

What is the difference between *wide format* data and *long format* data?

Wide format data is when each respondent is given their own row and column, in long response data, a given respondent will be given many entries


Is the format `staff` data *wide* or *long*?

Wide as hell fr

How many observations are there in the data set?
5



## Exercise 2

```{r pivoting}
## Uncomment and complete the following code

staff_long <- read_csv("data/instructional-staff.csv") %>%
  pivot_longer(
    cols = 5, 
    names_to = "Year",
    values_to = "percent"
    )

```


## Exercise 3

```{r dot-plot}
# Write your code for Exercise 3 here



```

## Exercise 4

```{r bad-line-plot}
# Write your code for Exercise 4 here



```

## Exercise 5

```{r mutate-year}
## Uncomment and complete the following code

#staff_long <- staff_long %>%
#  mutate(year = ______(year))
```


```{r working-line-plot}
# Write your code for Exercise 5 here



```

## Exercise 6

```{r better-line-plot}
# Write your code for Exercise 6 here




```

## Exercise 7

_Write below your ideas for adapting the line plot_

* Idea 1
* Idea 2
* Idea 3
* ...


## Exercise 8

```{r implement-Ex7}
# Write your code for Exercise 8 here



```

